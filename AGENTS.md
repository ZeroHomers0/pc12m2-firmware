...
- **2026-09-01/02 后续进度**：产品信息定制（型号/版本/厂商/电话）、厂商 X/O 字形修复与
  ...
  `docs/analysis/P5_VERIFICATION_PROGRESS.md`。当前历史基线 `text 70584 / data 6312 / bss 2196`；
  本次修复后构建为 `text 70736 / data 6312 / bss 2196`，`firmware.bin` SHA-256
  `C62683D6B1D714E1390977F3E951ACFD53BC6D99FABF42D9FF7E953D81E13494`。
...
- **2026-09-21 显示重绘修复**：字符串渲染按 LCD 列而非字节数限制，避免 ASCII/GBK 混排的
  `10.语言选择` 截断、残留和选中背景不完整；参数编辑超时清除数值后重新绘制 V/A/° 单位，
  确保中英文界面单位在重绘后保持可见。A/B 显示矩阵和全执行验证均 PASS。
- **2026-09-22 双语代码整理**：数值域固定为列 11..14、单位固定为列 15，删除编辑超时路径的
  重复补单位代码；原厂单位绘制调用保留，12P 也保留 `10.` 后中文所需的奇数列汉字渲染器。维护不变量见
  `docs/analysis/BILINGUAL_UI_MAINTENANCE.md`。
