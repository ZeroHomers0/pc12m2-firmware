...
/* 0x00001528 —— 输入引脚方向配置（置输入模式；12p）
 *   DAT_00001924=0x2009C000（FIO 池基址，12p globals 为标量，需强转）
 *   [8]  = FIO1DIR（+0x20）；[0x18]=FIO2DIR（+0x60）
 *   配置为输入（13 次 bic，与 dump 0x1528 一一对应）：P1.17/16（0x80000/0x40000）、
 *   P0.30/29（0x40000000/0x20000000）、P2.25/24（0x2000000/0x4000000）、
 *   P0.27/28（0x8000000/0x10000000）、P1.16/17（0x10000/0x20000）、
 *   P0.6（0x40）、P0.3（0x4）、P0.2（0x8） */
void gpio_inputs_dir_init(void)
{
  volatile uint32_t *fio = (volatile uint32_t *)DAT_00001924;   /* 0x2009C000 */

  fio[8] = fio[8] & 0xfff7ffff;                   /* P1.17 输入 */
...
undefined1 input_scan_state(void)
{
  ...
  scan_cnt = DAT_00001928;
  /* —— 任一输入未就绪（P1.16/P1.17/P0.28/P0.27/P2.9/P2.8 为 0）→ 进入扫描 —— */
  if ((((((*(volatile uint *)((uint32_t)DAT_00001924 + 0x34) & 0x80000) == 0) ||
        ((*(volatile uint *)((uint32_t)DAT_00001924 + 0x34) & 0x40000) == 0)) ||
       ((*(volatile uint *)((uint32_t)DAT_00001924 + 0x14) & 0x40000000) == 0)) ||
      (((*(volatile uint *)((uint32_t)DAT_00001924 + 0x14) & 0x20000000) == 0 ||
       ((*(volatile uint *)((uint32_t)DAT_00001924 + 0x74) & 0x2000000) == 0)))) ||
     ((*(volatile uint *)((uint32_t)DAT_00001924 + 0x74) & 0x4000000) == 0)) {
    *DAT_00001928 = *DAT_00001928 + 1;
    if (*scan_cnt == 0x19) {
    ...
    if (0xf9 < *DAT_00001928) {
      ...
      *DAT_0000192c = 0;
      ...
      if ((((*(volatile uint *)((uint32_t)DAT_00001924 + 0x34) & 0x80000) == 0) &&
         ...
         ((((*(volatile uint *)((uint32_t)DAT_00001924 + 0x14) & 0x40000000) != 0 &&
           (((*(volatile uint *)((uint32_t)DAT_00001924 + 0x14) & 0x20000000) == 0 &&
            ((*(volatile uint *)(DAT_00001bbc + 0x74) & 0x2000000) != 0)))) &&
          ((*(volatile uint *)(DAT_00001bbc + 0x74) & 0x4000000) != 0)))) {
        *DAT_00001bc0 = *DAT_00001bc0 + 1;
        if (0x1f < *cnt_p) {
          *cnt_p = 0x1f;
        }
        if (*DAT_00001bc0 == 0x1e) {
    ...
    /* —— 全部输入就绪（稳定）—— */
    if (0x18 < *DAT_00001928) {
      *DAT_00001928 = 0;
      return *DAT_0000192c;    /* 返回锁存方向 */
    }
    *DAT_0000192c = 0;
    *DAT_00001928 = 0;
    *DAT_00001930 = 0;
...
undefined1 scan_run_stop(void)
{
  ...
  if (*eeprom_param_1 == '\0') {
    /* —— 单次触发模式 —— */
    if (((*(volatile uint *)(DAT_00001bbc + 0x14) & 0x10000000) == 0) &&
       ((*(volatile uint *)(DAT_00001bbc + 0x14) & 0x8000000) != 0)) {
      *DAT_00001bc8 = *DAT_00001bc8 + 1;
      if (*cnt_p == 0x32) {
        *cnt_p = 0x32;
        return 7;              /* RUN */
      }
      if (0x32 < *DAT_00001bc8) {
        *DAT_00001bc8 = 0x32;
      }
    }
    else {
    ...
    if (((*(volatile uint *)(DAT_00001bbc + 0x14) & 0x10000000) == 0) ||
      ...
      *DAT_00001bcc = *DAT_00001bcc + 1;
      if (*cnt_p == 0x32) {
        *cnt_p = 0x32;
        return 8;              /* STOP */
      }
      if (0x32 < *DAT_00001bcc) {
        *DAT_00001bcc = 0x32;
      }
    }
    evt = 0;
...
undefined4 debounce_p09(void)
{
  ...
  p_cnt = DAT_00001bd4;
  if ((*(volatile uint *)(DAT_00001bbc + 0x14) & 0x200) == 0) {   /* P0.9 低 */
    *DAT_00001bd4 = 0;
  }
  else {
    *DAT_00001bd4 = *DAT_00001bd4 + 1;
    if (0xf < *p_cnt) {
      *p_cnt = 0;
      return 1;
    }
...
undefined4 chk_p02_p03(void)
{
  ...
  if (((*(volatile uint *)(DAT_00001bbc + 0x14) & 4) == 0) && ((*(volatile uint *)(DAT_00001bbc + 0x14) & 8) == 0)) {
    ret = 1;
  }
  else {
    ret = 0;
